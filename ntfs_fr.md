...
trip_id | chaine | Requis | Identifiant de la circulation
arrival_time | heure | Optionnel | Heure d'arrivée. Si l'heure d'arrivée n'est pas connue, elle doit être estimée par le système fournissant les données et le champ *stop_time_precision* doit être spécifié à 1. Si la descente est interdite à cet arrêt, l'heure d'arrivée doit être indiquée et le champ *drop_off_type* doit être spécifié à 1. La valeur doit être obligatoirement nulle si *start_pickup_drop_off_window* ou *end_pickup_drop_off_window* est défini.
departure_time | heure | Optionnel | Heure de départ. Si l'heure de départ n'est pas connue, elle doit être estimée par le système fournissant les données et le champ *stop_time_precision* doit être spécifié à 1. Si la montée est interdite à cet arrêt, l'heure de départ doit être indiquée et le champ *pickup_type* doit être spécifié à 1. La valeur doit être obligatoirement nulle si *start_pickup_drop_off_window* ou *end_pickup_drop_off_window* est défini.
start_pickup_drop_off_window | heure | Optionnel | Heure d'ouverture du service de TAD sur le point d'arrêt. Valeur obligatoire si *end_pickup_drop_off_window* est défini, nulle si *arrival_time* ou *departure_time* est défini.
end_pickup_drop_off_window | heure | Optionnel | Heure de fermeture du service de TAD sur le point d'arrêt. Valeur obligatoire si *start_pickup_drop_off_window* est défini, nulle si *arrival_time* ou *departure_time* est défini.
